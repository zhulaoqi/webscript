...
class InVideoDetailScraper(DetailScraperBase):
    """InVideo 详情页爬虫"""
    
    def __init__(self, data_manager, target_count: int = 50):
        super().__init__(data_manager, target_count)
        self.base_url = 'https://invideo.io/ideas'
        self.category_name = 'InVideo'
    
    def scrape(self) -> int:
        """执行爬取"""
        print(f"\n开始爬取 InVideo.io (目标: {self.target_count} 条)")
    ...
    def _extract_detail_info(self) -> dict:
        ...
        try:
            ...
            # 方法2: script/description/idea相关class
            if not prompt:
                try:
                    containers = self.driver.find_elements(By.XPATH, 
                        "//*[contains(@class, 'script') or contains(@class, 'description') or contains(@class, 'idea') or contains(@class, 'prompt')]")
            ...
            if not prompt:
                try:
                    texts = self.driver.find_elements(By.XPATH, "//p | //div")
            ...
            # InVideo通常是文生视频，但也检查原图 - 使用严格过滤
            original_image_url = None
            try:
                candidates = []
                for keyword in ['input', 'reference']:
                        ...
                        try:
                            ...
                            for img in imgs[:5]:
                                try:
                                    ...
                                    if not src or 'invideo' not in src.lower():
                                        continue
                                    
                                    # 排除UI元素
                                    exclude_keywords = [
                                        'logo', 'avatar', 'profile', 'icon', 'favicon',
                                        'price', 'pricing', 'banner', 'ad', 'button',
                                        'nav', 'menu', 'badge', 'tag', 'coin'
                                    ]
            ...
            if not cover_url:
                try:
                    ...
                    for img in imgs:
                        src = img.get_attribute('src')
                        if src and 'invideo' in src.lower():
                            if not any(p in src.lower() for p in ['profile', 'avatar', 'logo']):
